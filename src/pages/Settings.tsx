...
const BIDDING_FIELDS: FieldConfig[] = [
  ...
  { key: 'min_bid', label: 'Min bid', description: 'Floor for recommended bid changes.', type: 'number', min: 0.01, step: 0.01 },
  { key: 'max_bid', label: 'Max bid', description: 'Ceiling for recommended bid changes.', type: 'number', min: 0.01, step: 0.01 },
...
const OPERATION_FIELDS: FieldConfig[] = [
  ...
  { key: 'max_daily_loss', label: 'Max daily loss', description: 'Loss threshold for review alerts.', type: 'number', min: 0, step: 10 },
  { key: 'currency', label: 'Currency', description: 'Display currency code.', type: 'text' },
  {
    key: 'action_mode',
    label: 'Account mode',
    description: 'Task 1 safe modes only.',
    type: 'select',
    options: [
...
export function SettingsPage() {
  ...
  const addSource = () => {
    ...
    setForm((prev) => ({
      ...prev,
      account_sources: [...prev.account_sources, source],
    ...
    setForm((prev) => {
      ...
      return {
        ...
        selected_account_source_id:
          prev.selected_account_source_id === id ? nextSources[0]?.id ?? '' : prev.selected_account_source_id,
      };
    });
    setSaved(false);
    setDangerNote(null);
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Settings"
        description="Account health, data source status, and safe operating controls"
        actions={
      ...
      <section className="grid grid-cols-1 xl:grid-cols-4 gap-4">
        {previewHealth.accountCards.map((card) => (
          <AccountHealthCard key={card.id} card={card} />
        ))}
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-4">
        <Card className="xl:col-span-2">
          <CardHeader
            title="Per-Account Controls"
            actions={<ModeBadge mode={previewHealth.safeMode} />}
          />
          <CardBody>
            ...
            <div className={`mt-4 border rounded-lg px-3 py-2 text-sm ${TONE_CLASSES[previewHealth.automationDisabled ? 'safe' : 'ok']}`}>
              <div className="flex items-center gap-2 font-medium">
                <ShieldAlert size={14} />
                {previewHealth.automationDisabled ? 'Kill switch is active' : 'Kill switch is visible and ready'}
              </div>
              <p className="mt-1 text-xs">
                Current saved mode is {labelForMode(savedHealth.safeMode)}. This page only stores local settings and read-only import configuration.
              </p>
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          ...
          <CardBody className="space-y-3">
            ...
            <div className="border rounded-lg border-gray-200 p-3 text-xs text-gray-600">
              ...
              <p className="mt-1">Only masked values and placeholders are rendered here.</p>
            </div>
          </CardBody>
        </Card>
      </section>
      ...
      <section className="mt-4">
        <Card>
          <CardHeader
            title="Connected Account Sources"
            actions={
              <button
                type="button"
                onClick={addSource}
          ...
          <CardBody className="space-y-4">
            <div className="flex flex-col gap-2 md:flex-row">
              <select
                value={form.selected_account_source_id}
                onChange={(event) => handleChange('selected_account_source_id', event.target.value)}
                className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 bg-white"
              >
                <option value="">
                  {form.account_sources.length === 0 ? 'No account sources' : 'Select account source'}
                </option>
                {form.account_sources.map((source) => (
                  <option key={source.id} value={source.id}>
                    {source.account_name || source.account_id || 'Unnamed account'} ({source.customer_id || 'no customer ID'})
                  </option>
                ))}
              </select>
              <span className="rounded-lg border border-gray-200 px-3 py-2 text-xs text-gray-500">
                {form.account_sources.filter((source) => source.enabled).length}/{form.account_sources.length} enabled
              </span>
            </div>
            ...
            {selectedSource ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border border-gray-100 rounded-lg p-3">
                ...
                {renderSourceField(selectedSource, 'customer_id', 'Google Ads customer ID')}
                ...
                <label className="flex items-center gap-2 text-sm text-gray-700">
                  ...
                  />
                  Enabled for local imports
                </label>
                <button
                  type="button"
                  onClick={() => removeSource(selectedSource.id)}
                  className="inline-flex w-fit items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-red-600 bg-white rounded-lg border border-red-200 hover:bg-red-50 transition-colors"
                >
                  <Trash2 size={12} />
                  Remove local source
                </button>
              ...
              <div className="rounded-lg p-3 text-sm bg-gray-50 text-gray-600">
                No account source configured. Add a source to track account health without storing credentials.
              </div>
            )}
          </CardBody>
        </Card>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-4">
        <Card className="xl:col-span-2">
          <CardHeader
            ...
            actions={
              <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full border text-xs ${TONE_CLASSES[previewHealth.syncTone]}`}>
                <span className={`h-2 w-2 rounded-full ${DOT_CLASSES[previewHealth.syncTone]}`} />
                {previewHealth.syncLabel}
              </span>
            }
          />
          <CardBody>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {previewHealth.dataSources.map((source) => (
                <DataSourceTile key={source.key} source={source} />
              ))}
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardHeader
            ...
            actions={<span className="text-xs text-gray-500">{previewHealth.latestScriptVersion ?? 'not reported'}</span>}
          />
          <CardBody className="space-y-3">
            <div className="border rounded-lg border-gray-200 p-3 bg-gray-50">
              <div className="flex items-center gap-2 text-sm font-medium text-gray-800">
                <FileText size={15} />
                Update flow
              </div>
              <pre className="mt-2 whitespace-pre-wrap text-[11px] leading-5 text-gray-600">{scriptCopy}</pre>
            </div>
            <button
              type="button"
              onClick={handleCopyScriptUpdate}
              className="inline-flex w-full items-center justify-center gap-2 px-3 py-2 text-xs font-medium bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors"
            >
              <Clipboard size={13} />
              {copied ? 'Copied' : 'Copy Script Update'}
            </button>
          </CardBody>
        </Card>
      </section>
          ...
          <CardBody>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <DangerButton
                ...
                title={DANGER_ACTIONS[0].label}
                description="Persist local mode as disabled."
                onClick={handleDisableAutomation}
              />
              <DangerButton
                icon={<RefreshCw size={15} />}
                title={DANGER_ACTIONS[1].label}
                description="Placeholder only; no token call."
                onClick={handleRotateFeedToken}
              />
              <DangerButton
                icon={<Unplug size={15} />}
                title={DANGER_ACTIONS[2].label}
                description="Stage local feed disconnect."
                onClick={handleDisconnect}
              />
            </div>
            {dangerNote && (
              <div className="mt-3 border rounded-lg border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                {dangerNote}
              </div>
            )}
          </CardBody>
    ...
    return (
      <div key={key}>
        ...
        <input
          type="text"
          value={String(value)}
          onChange={(event) => updateSource(source.id, { [key]: event.target.value } as Partial<AccountSource>)}
          className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
    );
    ...
    <Card>
      <CardBody>
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full border text-xs font-medium ${TONE_CLASSES[card.tone]}`}>
            {card.tone === 'warn' ? <AlertTriangle size={12} /> : <CheckCircle2 size={12} />}
            {card.status}
          </span>
        </div>
        <dl className="mt-4 grid grid-cols-2 gap-3">
          {card.values.map((item) => (
            <div key={item.label}>
              <dt className="text-[11px] uppercase tracking-wide text-gray-400">{item.label}</dt>
              <dd className="mt-0.5 text-sm font-medium text-gray-800 break-words">{item.value}</dd>
            </div>
          ))}
        </dl>
      </CardBody>
    ...
    <div className="border border-gray-200 rounded-lg p-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-sm font-medium text-gray-800 truncate">{source.label}</div>
          <div className="text-xs text-gray-500 truncate">{source.tabName}</div>
        </div>
        <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full border text-xs ${TONE_CLASSES[source.tone]}`}>
          <span className={`h-2 w-2 rounded-full ${DOT_CLASSES[source.tone]}`} />
          {source.rows > 0 ? `${source.rows} rows` : source.optional ? 'optional' : 'missing'}
        </span>
      </div>
      <div className="mt-2 text-xs text-gray-500">
        {source.importedAt ? `Imported ${new Date(source.importedAt).toLocaleString()}` : 'No local import metadata'}
      </div>
  ...
  return (
    <div className="flex items-center justify-between gap-3 border border-gray-200 rounded-lg px-3 py-2">
      <div className="flex items-center gap-2 min-w-0">
        <PlugZap size={14} className="text-gray-400 flex-shrink-0" />
        <span className="text-sm text-gray-700 truncate">{label}</span>
      </div>
      <code className="text-xs text-gray-500 bg-gray-50 rounded px-2 py-1 max-w-[55%] truncate">
        {value === 'server-managed-placeholder' ? 'server secret only' : maskSensitiveValue(value)}
      </code>
    </div>
  );
}
...
function DangerButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="text-left border border-red-200 bg-red-50 hover:bg-red-100 rounded-lg px-3 py-3 transition-colors"
    >
...
function ModeBadge({ mode }: { mode: SafeActionMode }) {
  return (
    <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full border text-xs ${TONE_CLASSES[mode === 'disabled' ? 'safe' : 'ok']}`}>
      <ShieldAlert size={12} />
      {labelForMode(mode)}
    </span>
  );
