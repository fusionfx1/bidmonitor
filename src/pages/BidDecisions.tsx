...
export function BidDecisions() {
    ...
    { key: 'reason', label: 'Reason', render: (r) => <span className="text-xs text-gray-500 max-w-[200px] block">{r.reason}</span>, sortable: false },
    {
      ...
      render: (r) => (
        <select
          ...
          onChange={(e) => setBidApproval(r.keyword_key, e.target.value as ApprovalStatus)}
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Bid Decisions"
        description="Read-only recommendations. No bids are changed automatically."
        actions={
          decisions.length > 0 ? (
            <ExportButton label="Export CSV" onClick={() => exportBidDecisionsCSV(decisions)} />
          ) : undefined
        }
      />

      {settings.action_mode === 'disabled' && (
        <div className="bg-gray-100 border border-gray-200 rounded-xl p-4 text-sm text-gray-600 mb-4 flex items-center gap-2">
          <AlertTriangle size={16} className="flex-shrink-0" />
          Auto-bid mode is disabled. Enable it in Settings to see actionable recommendations.
        </div>
      )}
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {actionFilters.map((action) => (
          ...
          >
            {action.replace(/_/g, ' ')} {actionCounts[action] != null ? `(${actionCounts[action]})` : ''}
          </button>
        ))}
      </div>
      ...
      <Card>
        <CardBody className="p-0 px-5 py-4">
          <DataTable
            columns={columns}
            data={filtered}
            emptyMessage="No bid decisions generated. Import google_keywords and google_auction_proxy_keywords CSVs."
            rowKey={(r) => (r as unknown as BidDecision).keyword_key}
          />
        </CardBody>
      </Card>
    </PageContainer>
